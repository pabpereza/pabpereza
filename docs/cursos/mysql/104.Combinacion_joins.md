...
keywords:
  ...
  - right join mysql
  - full join mysql
  - relacionar tablas sql
  - cómo usar joins mysql
  - cross join mysql
  - self join mysql
  - multiple joins mysql
  - join con where mysql
...
## JOIN
El `JOIN` es una forma de relacionar dos tablas y tiene varios comandos. La consulta anterior es un ejemplo de `INNER JOIN` realizado con un `WHERE`. Existen varios tipos de `JOIN` en función de la información que queramos obtener de dos tablas relacionadas.
