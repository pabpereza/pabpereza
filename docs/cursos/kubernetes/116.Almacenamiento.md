...
Esto es así en Kubernetes porque las aplicaciones desplegadas en pods son efímeras y no deben depender del almacenamiento local del nodo. Imaginemos que un pod se elimina o se mueve a otro nodo: si el almacenamiento estuviera en el nodo local, los datos se perderían o quedarían inaccesibles.
...
> Consejo: Antes de desplegar aplicaciones con estado (stateful) validemos requisitos de IOPS, latencia y disponibilidad para escoger la clase de almacenamiento correcta.
...
Podemos restringir cuánto almacenamiento pide un namespace y cuántos PVC puede crear.
...
## Buenas Prácticas
1. Definamos una StorageClass por perfil (rendimiento, backup, criptografía).
2. Usemos `WaitForFirstConsumer` en entornos multi-zona para evitar volúmenes en zonas erróneas.
3. Apliquemos etiquetas en PV estáticos para control fino de binding.
4. Documentemos políticas de `Retain` para evitar acumulación de volúmenes huérfanos.
5. Monitoricemos el estado de PVC (`Pending`) como indicador de problemas de aprovisionamiento.
6. Usemos `StatefulSet` para aplicaciones que requieren volúmenes persistentes por réplica.
