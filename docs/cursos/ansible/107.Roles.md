...
[![Roles y Ansible Galaxy: reutiliza tu código - Curso Ansible #7](https://img.youtube.com/vi/5tH-uroFfW8/maxresdefault.jpg)](https://youtu.be/5tH-uroFfW8)
...
#### Código espagueti vs receta de espaguetis
Imaginemos improvisar la misma receta cada vez que cocinamos, sin apuntar cantidades ni pasos: un día sale salada, otro se nos queda caldosa, en fin. Eso pasa también con un playbook gigante sin estructura: funciona, pero cada vez sale un poco distinto y nadie se atreve a tocarlo.

Los **roles** son como una **receta de cocina bien escrita**.
*   Tenemos una cocina donde preparamos los platos, que serían los servidores.
*   Queremos preparar un plato concreto (servidor web).
*   En lugar de improvisar los pasos cada vez, seguimos la receta "Nginx a la plancha" (rol `webserver`), con sus pasos fijos (`tasks`) y sus cantidades de ingredientes por defecto (`defaults`).
*   ¡Listo! El mismo plato, servido igual en cualquier cocina, ajustando solo la sal a nuestro gusto (variables).
...
Supongamos que ya tenemos el playbook de la [práctica del módulo 3](103.Playbooks.md): instala Nginx, configura el firewall con `ufw` y publica una página personalizada:
...
Funciona, pero todo vive apelmazado en un único fichero: instalación, firewall, configuración y contenido mezclados. Si mañana queremos este mismo servidor en otro proyecto, copiamos y pegamos las 60 líneas enteras. **Eso es espagueti.**

Ahora, "descuartizamos" las tareas y ponemos cada cosa en su lugar dentro de la carpeta `webserver/`:
...
- name: Crear página web personalizada
  copy:
    dest: "{{ webserver_html_root }}/index.html"
    content: |
      <h1>¡Hola desde Ansible! 🚀</h1>
      <p>Este servidor ha sido configurado automáticamente.</p>
      <p>SO: {{ ansible_facts['distribution'] }} {{ ansible_facts['distribution_version'] }}</p>
...
webserver_packages:
  - nginx
  - ufw
webserver_allowed_ports:
  - "22"
  - "80"
  - "443"
webserver_html_root: /var/www/html
```

**4. `roles/webserver/templates/nginx.conf.j2`**

El `./nginx.conf` estático que copiábamos a pelo ahora vive dentro del rol como plantilla Jinja2. Aunque hoy no interpole ninguna variable, en cuanto necesitemos que cambie por entorno (workers, límites, dominio) ya está en el sitio correcto.
...
¡Y si mañana queremos abrir un puerto extra o cambiar los paquetes instalados, tocamos `webserver_allowed_ports` o `webserver_packages` en un sitio y listo! Podemos usar el rol `webserver` en cualquier proyecto simplemente añadiéndolo a nuestro `requirements.yml`.


### Usar un rol desde `site.yml`

Una vez creado el rol, por ejemplo con `ansible-galaxy init roles/webserver`, podemos llamarlo desde el playbook principal usando únicamente el nombre de la carpeta del rol:
...
Con la estructura habitual `roles/webserver/`, Ansible lo encuentra automáticamente al ejecutar `ansible-playbook site.yml` desde la raíz del proyecto. Si el rol está en otra ubicación, debemos añadir esa ruta en `ansible.cfg` mediante `roles_path`:

```ini
[defaults]
roles_path = ./roles:../roles-compartidos
```

Ansible buscará primero en `./roles/` y después en `../roles-compartidos/`. Las rutas se separan con `:` en macOS y Linux. En Windows se separan con `;`.

### Dependencias entre roles

Los roles pueden depender de otros roles. Por ejemplo, si nuestro rol `webserver` necesita que PostgreSQL esté configurado antes de publicar la aplicación, podemos declarar esa dependencia en el archivo `meta/main.yml` del propio rol `webserver`.

#### 🔗 Ejemplo de dependencias

La estructura mínima del proyecto sería:
...
El nombre `postgresql` debe coincidir exactamente con el nombre de la carpeta del rol. La dependencia se ejecuta antes de las tareas de `webserver`, pero no se instala automáticamente: ambos roles deben existir en una ruta incluida en `roles_path`.

Si el rol `postgresql` utiliza la collection `community.docker`, necesitamos instalarla en el nodo de control. Ni `ansible-galaxy install` del rol ni `dependencies:` de `meta/main.yml` descargan collections automáticamente, así que la declaramos en `requirements.yml`:

**`requirements.yml`**
```yaml
# Los roles postgresql y webserver son locales: viven en ./roles/,
# no hace falta listarlos aquí.

# Collections que necesitan las tasks de esos roles
collections:
...
#### ¿Cómo funciona?
1.  Cuando ejecutamos el rol `webserver`, Ansible primero ejecuta `postgresql` y después las tareas propias de `webserver`.
...
- hosts: servers
  roles:
    - webserver  # Ejecutará postgresql → webserver
```

#### Ejecutando el conjunto

1.  **Instalamos las collections del proyecto** (los roles locales ya están en `./roles/`, no hace falta "instalarlos"):
    ```bash
    ansible-galaxy install -r requirements.yml
    ```
2.  **Comprobamos el `roles_path`**: `postgresql` y `webserver` deben convivir en una ruta incluida en `roles_path` (normalmente `./roles/`, configurada en `ansible.cfg`). Las `dependencies:` de `meta/main.yml` se resuelven en local: si `postgresql` no está ahí, el playbook falla al arrancar, no se descarga sobre la marcha.
3.  **Lanzamos el playbook** como cualquier otro:
    ```bash
    ansible-playbook -i inventory site.yml
    ```
4.  Ansible construye el grafo de dependencias de `webserver` y ejecuta, en orden: `postgresql` (con las variables de base de datos) → las tareas propias de `webserver`.
5.  Como cada rol es idempotente, si volvemos a lanzar el mismo comando no se recrea la base de datos ni se repiten las configuraciones ya aplicadas; Ansible solo hace los cambios que falten.
...
galaxy_info:
  author: Tu Nombre
  description: Instalación y configuración de Nginx como servidor web
...
Instala y configura Nginx como servidor web, con el firewall gestionado por `ufw`.

## Requisitos

- Ansible >= 2.14
- Hosts Debian/Ubuntu con `apt`

## Variables

| Variable | Default | Descripción |
|----------|---------|-------------|
| `webserver_packages` | `[nginx, ufw]` | Paquetes a instalar |
| `webserver_allowed_ports` | `["22", "80", "443"]` | Puertos permitidos en el firewall |
| `webserver_html_root` | `/var/www/html` | Directorio con el HTML servido |
...
3. Vamos a "Roles" → "Import role"
4. Seleccionamos nuestro repositorio `webserver`
