...
### SSH 
Te recomiendo cargar tu clave pública ssh para evitar usar la contraseña. En los proximos pasos, activaremos una protección de bruta bruta y, si pones mal la constraseña podría benearte el servidor. Mejor no tener que usarla, por si acasO.

Puedes compiar la clave pública SSH local al servidor remoto con el siguiente comando:
```bash
...
PD: Si no tuvieras clave ssh local, puedes creala con el comando `ssh-keygen`
...
ufw allow 22/tcp      # SSH — tu nuevo puerto, PRIMERO
ufw enable
ufw status verbose
```
...
Si necesitas acceso remoto al gateway, utiliza el comando ssh que devuelve `openclaw dashboard` para hacer un tunel inverso sin abrir puertos.
