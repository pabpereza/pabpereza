...
keywords:
  ...
  - distroless docker
  - imágenes distroless
  - reducir tamaño imagen docker
  - optimizar dockerfile
  - seguridad imágenes docker
  - scratch docker
  - google distroless
  - imagen docker minimal
  - construir imagen ligera docker
  - buenas prácticas dockerfile seguridad
slug: dockerfiles_multi_stage_y_distroless
---

# Dockerfiles Multi-Stage y Distroless
Si nuestras imágenes de contenedor pesan 1Gb o 2Gb, seguramente no solo tengamos problemas de rendimiento, también estamos arrastrando muchos paquetes del sistema operativo o librerías de terceros innecesarias.
...
Los Dockerfiles multi-stage son una característica de los contenedores OCI que permite utilizar múltiples instrucciones `FROM` en un mismo Dockerfile. Cada instruccción `FROM` inicia una nueva etapa de construcción, y podemos copiar selectivamente artefactos de una etapa a otra.
...
Estas imágenes están pensadas para ejecutar con lo mínimo necesario, están basadas en debian y, por no tener, no tienen ni intérprete de comandos. Esto último a veces hace más complicado de depurar las imágenes en producción. Para esto recordemos el artículo de [Docker Debug](https://pabpereza.dev/blog/debug_docker_build_vscode), que si hay interés, también podría extenderlo a kubernetes.
...
Esto debería funcionar como un tiro y ocupará apenas unos cuantos MB. Ahora, como decía antes, no siempre podemos usar scratch. Recomendamos probar las imágenes distroless y contar qué tal nuestra experiencia.
