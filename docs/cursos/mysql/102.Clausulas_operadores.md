---
title: "Cláusulas y Operadores en SQL: Filtrado y Ordenación"
description: "Domina las cláusulas WHERE, ORDER BY, GROUP BY y LIMIT para realizar consultas precisas en MySQL."
keywords:
  - sql clausulas
  - mysql where
  - mysql order by
...
sidebar_label: "Cláusulas y Operadores"
---
...
Ejemplo order by:
Por defecto, `ORDER BY` devolvería todos los registros la tabla `tabla ordenados por el `id` de forma ascendente (sin especificar nada), también podríamos usar el `DESC` para ordenarlas de forma ascendent
```sql
