...
El nivel 1 ya resuelve el 80% del problema y cuesta una tarde. **No te saltes el nivel 1 esperando a montar el nivel 2**: mucha gente lleva años con secretos en el código porque Vault le parecía mucho trabajo.
...
Tienes la interfaz web en `http://localhost:8200` — entra con el token `root`.
...
Ese `version` importa: Vault guarda el histórico, así que puedes volver a una versión anterior si una rotación sale mal.
...
El `-field` devuelve el valor pelado, que es lo que necesitas para inyectarlo en un script:
...
Aquí llega la pregunta incómoda: para leer de Vault necesitas un token. ¿Y dónde guardas ese token?

Si acabas con el token de Vault escrito en el código, has movido el problema, no lo has resuelto.
...
La forma más cómoda de que esto llegue a tus aplicaciones sin que ellas sepan nada de Vault. El operador lee del gestor y mantiene un `Secret` nativo sincronizado:
...
Tu Deployment consume un `Secret` normal y corriente. La aplicación no lleva ni una línea de código de Vault, y el secreto se refresca solo cada hora.
...
Por eso importa tanto el RBAC. Lo tienes en el [capítulo de seguridad del curso de Kubernetes](../kubernetes/119.Seguridad.md).
...
| **AWS Secrets Manager** | Ya estás en AWS y no quieres operar nada |
...
Si estás en una sola nube, empieza por el gestor de tu proveedor: cero operación y se integra solo con los permisos que ya tienes. Vault gana cuando necesitas credenciales dinámicas o vives en varias nubes.
...
- Nivel 1 (variables de entorno) ya resuelve casi todo y cuesta una tarde: no lo pospongas
