...
Como vemos, todos estos objetos se gestionan de forma similar a los deployments: solo hay que especificar el tipo de objeto después de las instrucciones get, edit, describe o delete.
...
¿Ya nos imaginamos por dónde van los tiros? Efectivamente, esto es ideal para bases de datos o sistemas con almacenamiento persistente.
...
Fijémonos en el campo `serviceName`: los StatefulSets necesitan un servicio "headless" asociado, que es el que proporciona a cada pod su nombre DNS estable. Veremos los servicios en detalle en el [capítulo de Services](./111.Services.md).

Este ejemplo estaría incompleto, ya que los volúmenes los veremos en capítulos posteriores ([Almacenamiento y volúmenes en Kubernetes](./116.Almacenamiento.md)), pero sirve para que veamos la estructura básica de un StatefulSet.
...
No nos preocupemos, porque iremos viendo todos estos objetos en profundidad en los casos prácticos.
