...
```yaml
app_name: notastack-api
app_image: registry.local/notastack-api:latest
app_port: 8080
...
- name: Desplegar aplicación como contenedor con secretos
  ...
  tasks:
    - name: Autenticar en el registry con token del Vault
    ...
    - name: Desplegar contenedor con credenciales inyectadas como env vars
