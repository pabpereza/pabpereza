---
title: Ansible con Docker y Kubernetes - automatiza contenedores
description: >-
  Automatiza Docker y Kubernetes con Ansible. Despliega contenedores con
  community.docker, orquesta clústeres con kubernetes.core y olvida los
  scripts bash.
keywords:
  - ansible docker
  - ansible kubernetes
...
sidebar_label: 5. Ansible y contenedores
sidebar_position: 5
tags:
  - ansible
  - docker
...
Hasta ahora hemos usado Ansible para gestionar servidores tradicionales. En este capítulo damos el salto al mundo de los **contenedores**: aprendemos a orquestar Docker y Kubernetes desde Ansible, sustituyendo scripts bash frágiles por playbooks idempotentes y reutilizables.

[![Ansible + Docker + Kubernetes: automatiza tus contenedores - Curso Ansible #5](https://img.youtube.com/vi/7IoboMNhEYc/maxresdefault.jpg)](https://youtu.be/7IoboMNhEYc)

:::tip Cursos relacionados
Este capítulo asume que ya conocemos los fundamentos de contenedores. Si necesitamos repasarlos, tenemos el [curso de Docker](/docs/cursos/docker) y el [curso de Kubernetes](/docs/cursos/kubernetes) completos en esta web.
:::
...
|-----------|-------------------|----------------|
| Levantar un stack en local | `docker compose up` | — (lo usamos nativo) |
...
- name: Instalación de docker
  ...
  tasks:
    ...
    - name: Actualizar índice de paquetes tras añadir Docker
      apt:
        update_cache: yes
        cache_valid_time: 0
    ...
    - name: Instalar paquete pip para Python 3
      apt:
        name: python3-pip
        state: present
...
### Si `hello-world` falla con `overlay ... invalid argument`

Ese error no suele ser de `pip`. Lo normal es que el host no soporte bien `overlayfs` (bastante habitual en ciertos VPS/LXC/OpenVZ o kernels antiguos).

Diagnóstico rápido en el host destino:
...
Si nos sigue fallando con ese error, aplicamos este workaround reforzado. Forzamos `vfs` y desactivamos `containerd snapshotter` para evitar que Docker intente montar overlay igualmente:

```yaml
---
- name: Workaround overlayfs invalid argument
  hosts: all
  become: true

  tasks:
    - name: Parar Docker antes de reconfigurar
      service:
        name: docker
        state: stopped
    ...
    - name: Arrancar Docker con nueva configuración
      service:
        name: docker
        state: started
        enabled: yes

    - name: Recolectar info de Docker
      command: docker info
      register: docker_info
      changed_when: false

    - name: Mostrar storage driver activo
      debug:
        msg: "Driver activo: {{ docker_info.stdout | regex_search('Storage Driver: (.+)', '\\1') }}"

    - name: Verificar hello-world tras workaround
      command: docker run --rm hello-world
      register: hello_after_fix
      changed_when: false

    - name: Mostrar resultado de hello-world
      debug:
        var: hello_after_fix.stdout_lines
```

Si incluso con esto falla, el host probablemente no permite contenedores Docker completos (por ejemplo, LXC no privilegiado). En ese caso, usamos un host VM dedicado o activamos nesting/privilegios en el proveedor.

`vfs` funciona para salir del paso, pero penaliza rendimiento y consumo de disco. En producción, lo recomendable es mantener `overlay2` sobre un kernel/filesystem compatible.
...
Cuando queremos gestionar el build desde Ansible (por ejemplo, en un pipeline CI/CD propio), el módulo `docker_image` permite construir y hacer push a un registry. Aquí publicamos la imagen de `quotes` en GitHub Container Registry.
...
Necesitamos un `kubeconfig` válido en el host donde corre Ansible (normalmente el control node).
...
Este patrón —**manifiestos K8s renderizados con Jinja2 desde Ansible**— es muy potente: nos da variables, condicionales y bucles que Helm sólo ofrece de forma limitada, y se integra con nuestro inventario de Ansible para distinguir entornos (dev/staging/prod).
...
- **Usamos Vault** para tokens de registry, kubeconfigs y credenciales (lo vimos en el capítulo 7).
- **No reinventamos Helm**: si ya existe un chart oficial, lo instalamos desde Ansible en lugar de traducirlo a manifiestos sueltos.
- **Separamos bootstrap (infra) de despliegue (app)**: Ansible es excelente para el primero; para el segundo, consideramos usar GitOps (ArgoCD/Flux) y limitar Ansible al pipeline.
- **Idempotencia**: usamos siempre `state: present`/`absent` en lugar de `command: kubectl apply ...`.
- **Tags por entorno**: estructuramos nuestro playbook con tags `dev`, `staging`, `prod` y lo combinamos con inventarios separados.
...
Nos fijamos en cómo las variables de Ansible (`{{ mysql_password }}`, `{{ wp_port }}`) se inyectan directamente en el Compose. El template se renderiza en el host destino antes de pasárselo a Docker, sin necesidad de archivos `.env` separados.
