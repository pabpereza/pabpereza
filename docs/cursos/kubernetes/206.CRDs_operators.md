...
De hecho, ya hemos usado CRDs sin saberlo: los recursos de [Gateway API](./112.Gateway.md) (`Gateway`, `HTTPRoute`), los `NetworkPolicies` de Calico o los `VolumeSnapshots` del [capítulo de almacenamiento](./205.Almacenamiento_avanzado.md) son recursos personalizados.
...
> Si encontramos ejemplos con `apiVersion: apiextensions.k8s.io/v1beta1`, están desactualizados: esa versión fue eliminada en Kubernetes 1.22. La versión `v1` exige definir el esquema OpenAPI, que es lo que permite a Kubernetes validar nuestros objetos.
...
### Ejemplos reales
- **cert-manager**: definimos un recurso `Certificate` y el operator se encarga de pedirlo a Let's Encrypt, renovarlo y guardarlo en un Secret.
- **Prometheus Operator**: definimos un `ServiceMonitor` y el operator reconfigura Prometheus para monitorizar nuestro servicio.
- **CloudNativePG / MySQL Operator**: definimos un recurso `Cluster` y el operator despliega la base de datos, configura la replicación, gestiona el failover y los backups.

La experiencia de usuario es siempre la misma: en lugar de gestionar decenas de objetos de bajo nivel, declaramos **un** recurso de alto nivel y el operator hace el resto. Podemos explorar el catálogo de operators disponibles en [OperatorHub.io](https://operatorhub.io/).
...
- Gran parte del ecosistema que ya usamos (cert-manager, Gateway API, Cilium...) funciona sobre CRDs.
