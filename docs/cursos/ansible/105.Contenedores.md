...
Este capítulo asume que ya conoces los fundamentos de contenedores. Si necesitas repasarlos, tienes el [curso de Docker](/docs/cursos/docker) y el [curso de Kubernetes](/docs/cursos/kubernetes) completos en esta web.
:::
...
Si te sigue fallando con ese error, aplica este workaround reforzado. Fuerza `vfs` y desactiva `containerd snapshotter` para evitar que Docker intente montar overlay igualmente:
...
Si incluso con esto falla, el host probablemente no permite contenedores Docker completos (por ejemplo, LXC no privilegiado). En ese caso, usa un host VM dedicado o activa nesting/privilegios en el proveedor.
