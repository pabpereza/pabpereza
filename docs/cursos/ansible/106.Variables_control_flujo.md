---
title: Variables y facts en Ansible - precedencia, when y loop
description: >-
  Variables, facts, group_vars, host_vars, register y precedencia en Ansible,
  más when, loop y handlers avanzados para playbooks dinámicos. Con ejemplos.
keywords:
  - ansible variables
  - ansible facts
  - ansible group_vars
...
En el módulo 3 hemos visto cómo utilizar `when`, `loop` y handlers en su forma básica. Aquí vamos a ir un paso más allá: el sistema de variables de Ansible, los facts como fuente de verdad del sistema, y las herramientas avanzadas que convierten un playbook que se adapta a todo lo que le echemos.

[![Variables, Facts y control de flujo en Ansible (Curso Ansible #6)](https://img.youtube.com/vi/VgIFJiAj0e4/maxresdefault.jpg)](https://youtu.be/VgIFJiAj0e4)

## El sistema de variables

En el módulo 3 definimos variables directamente en el playbook con `vars:`. Funciona, pero no escala: si tenemos 50 hosts con configuraciones distintas, acabamos con un playbook ilegible lleno de condicionales.

La solución profesional son las carpetas `group_vars` y `host_vars`. Ansible las carga automáticamente sin que tengamos que importarlas.
...
**`vars.yml`** — no hay ningún valor hardcodeado:
```yaml
- hosts: madrid
  tasks:
...
Antes de ejecutar cualquier tarea, Ansible interroga cada host y almacena su estado en variables automáticas llamadas **facts**. Son nuestra fuente de verdad sobre el sistema real.
...
A veces necesitamos derivar un valor a partir de otros. `set_fact` crea una variable en tiempo de ejecución, disponible para el resto del play:
...
En playbooks que no necesitan facts (o cuando son lentos en hosts remotos), podemos desactivarlos:
...
`register` guarda el resultado de una tarea en una variable. Vimos su uso para idempotencia con `command` en el módulo anterior. Aquí va el uso completo.
...
### Caso real: verificando antes de actuar
...
Si definimos `http_port` en `group_vars`, en el playbook y también lo pasamos por línea de comandos, ¿cuál gana?
...
Las condiciones básicas ya las vimos en el módulo 3. Aquí van los patrones que usaremos en código real.
...
> Cuando combinamos `loop` con `register`, el resultado es una lista en `result.results[]`. Podemos filtrarla con `selectattr('failed', 'equalto', true)` para tomar decisiones basadas en qué elementos del loop fallaron.


## Handlers avanzados

El patrón básico de handlers (notify → ejecutar al final del play) está en el módulo 3. Aquí van los patrones que necesitamos en entornos reales.
...
En lugar de que cada tarea conozca el nombre exacto del handler, usamos un topic:
...
Por defecto los handlers se ejecutan al final. Si necesitamos que se ejecuten **ahora** (por ejemplo, el servicio debe estar corriendo antes de que la siguiente tarea lo use):
...
Por defecto, si el play falla antes del final, los handlers pendientes **no se ejecutan**. Útil en CI/CD donde queremos garantizar limpieza aunque haya error:
