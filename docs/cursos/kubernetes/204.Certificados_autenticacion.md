...
Para inspeccionar cualquier certificado, openssl es nuestra herramienta:
```bash
...
Y si renovamos `admin.conf`, recordemos actualizar nuestra copia local:
```bash
sudo cp /etc/kubernetes/admin.conf ~/.kube/config
```

> **Detalle importante:** `kubeadm upgrade` renueva automáticamente todos los certificados. Si actualizamos el cluster al menos una vez al año (deberíamos), nunca nos caducarán.
...
Los `signerName` que debemos reconocer: `kubernetes.io/kube-apiserver-client` (usuarios), `kubernetes.io/kube-apiserver-client-kubelet` (kubelets) y `kubernetes.io/kubelet-serving` (certificado de servidor del kubelet).
...
| `x509: certificate is valid for X, not Y` | Accedemos por una IP/nombre que no está en los SANs | Regenerar el certificado del API server añadiendo el SAN |
...
- Ante errores `x509`, leemos el mensaje completo: dice exactamente qué falla (caducidad, CA o SANs).
