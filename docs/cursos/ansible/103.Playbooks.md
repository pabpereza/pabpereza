---
title: Playbooks de Ansible - tutorial de sintaxis YAML y ejemplos
description: >-
  Aprende a escribir tu primer playbook de Ansible paso a paso. Sintaxis YAML,
  plays, tasks, handlers, when, loops y tags, con ejemplos reales explicados.
keywords:
  - ansible playbooks
  - yaml ansible
...
El corazón de Ansible: definir el estado deseado de nuestra infraestructura.

Estas piezas de código son nuestras "recetas" para configurar servidores, desplegar aplicaciones y orquestar tareas complejas. En este capítulo, vamos a diseccionar la sintaxis YAML, la estructura de un playbook y cómo ejecutar nuestras creaciones con `ansible-playbook`.


[![Playbooks en Ansible: sintaxis YAML y estructura - Curso Ansible #3](https://img.youtube.com/vi/F1GwuMWBKzo/maxresdefault.jpg)](https://youtu.be/F1GwuMWBKzo)
...
Hay dos estructuras clave que debemos dominar:
...
tasks:
    ...
    apt:
      name: nginx
```
*Si mezclamos tabs y espacios, Ansible nos odiará y el playbook fallará. Configuremos nuestro editor para usar solo espacios.*
...
Ansible tiene miles de módulos, pero usaremos estos 4 el 80% del tiempo. Los presentamos en el orden lógico de ejecución: primero el usuario, luego el software, luego la configuración y por último el servicio.

### Gestión de usuarios (`user`)
Creamos o modificamos usuarios del sistema. El usuario debe existir **antes** de instalar el software que va a correr bajo él.
...
Antes de instalar nada, actualizamos el índice de paquetes para que `apt` conozca las versiones más recientes disponibles. Sin este paso, podríamos instalar una versión antigua o encontrar errores de "package not found".
...
Subimos archivos desde nuestra máquina de control al servidor remoto. Asignamos propietario `nginx` para que el proceso pueda leer su configuración.
...
Ya tendríamos nuestro primer playbook completo y es el momento de ejecutarlo. Para este bloque, hemos creado en la raíz del proyecto un carpeta `nginx` con el playbook `nginx.yml` y el archivo de configuración `nginx.conf`.
...
Fijémonos en el parámetro `state`. En Ansible no decimos "Instala esto", decimos "Asegúrate de que esto esté **presente**".
*   Si no está -> Lo instala.
*   Si ya está -> **No hace nada**.

Esto es **Idempotencia**: Podemos ejecutar el mismo Playbook 1000 veces, y el resultado final siempre será el mismo, sin romper nada.
...
Cuando cambiamos una bombilla (task), necesitamos probar el interruptor (handler) para verificar que funciona. Pero si no cambiamos la bombilla, no tiene sentido probar el interruptor.
...
A veces necesitamos ejecutar una task solo si se cumple cierta condición. Para eso usamos `when`.

### 🚦 La analogía: el semáforo
No arrancamos el coche hasta que el semáforo está en verde. La condición es: "color == verde".
...
Al ejecutarlo con `ansible-playbook -i hosts.ini playbook.yml`, Ansible recopila los *facts* de cada host y evalúa la condición. En `target1` y `target2` veremos:
- `apt` → **ok** (son Ubuntu)
...
En una fábrica, la misma acción se repite para cada producto que pasa por la cinta. No creamos 100 estaciones iguales, usamos una y procesamos cada item.
...
**Equivalente sin loop (¡NO hagamos esto!):**
```yaml
- name: Instalar git
  apt:
    name: git
...
Los **tags** (etiquetas) nos permiten ejecutar solo ciertas tasks del playbook sin ejecutarlo completo.

### 🏷️ La analogía: filtros de email
Imaginemos que nuestro inbox tiene etiquetas: "Urgente", "Trabajo", "Personal". Podemos ver solo los emails con una etiqueta específica. En los playbooks es lo mismo, pero aplicado a cada una de las tareas.
...
## Práctica: nuestro primer servidor web 🌍
...
1.  Entramos al servidor y **modificamos** la configuración: `echo "test" >> /etc/nginx/sites-available/default`
2.  El servidor puede tener comportamiento inesperado.
3.  **Volvemos a ejecutar el Playbook con `--tags config`**

Veremos que Ansible detecta el cambio (Changed: 1), restaura la configuración correcta y ejecuta el handler para reiniciar Nginx. **Ha reparado el sistema automáticamente**.
...
**Próximo paso:** Variables y templates para hacer nuestros playbooks dinámicos y reutilizables 🎯
