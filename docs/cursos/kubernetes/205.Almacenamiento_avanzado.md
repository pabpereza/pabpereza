---
title: 'Volúmenes y Almacenamiento Avanzado en Kubernetes - CSI, Longhorn y Rook-Ceph'
description: >-
  Almacenamiento avanzado en Kubernetes para administradores - drivers CSI,
  Longhorn, Rook-Ceph, NFS, local PV, snapshots, clones y expansión de
  volúmenes para la certificación CKA.
sidebar_label: 205. Almacenamiento avanzado
slug: kubernetes_almacenamiento
image: 'https://pabpereza.dev/img/banner_kubernetes.png'
tags: [kubernetes, almacenamiento, devops, devsecops, CKA]
keywords:
  - kubernetes almacenamiento
  - pv pvc kubernetes
  - storageclass kubernetes
  - csi drivers kubernetes
  - longhorn kubernetes
  - rook ceph kubernetes
  - nfs kubernetes
  - snapshot kubernetes
  - clone volumen kubernetes
  - aprovisionamiento dinamico kubernetes
  - almacenamiento cloud kubernetes
  - aws ebs kubernetes
  - azure disk kubernetes
---

# Volúmenes y almacenamiento avanzado en Kubernetes

## Introducción
En la parte común del curso ya vimos los [fundamentos del almacenamiento persistente](./116.Almacenamiento.md) (PV, PVC, StorageClass). En esta lección, orientada a administradores, profundizaremos en cómo elegir la opción adecuada según el tamaño del clúster y nuestros SLOs (Service Level Objectives, los objetivos de nivel de servicio). Veremos en profundidad la diferencia entre aprovisionamiento **estático** y **dinámico**, el ciclo de vida de PV/PVC/StorageClass y cómo se relaciona con backends: almacenamiento nativo de nube, **Longhorn** (entornos pequeños), **Rook‑Ceph** (entornos medianos), además de **NFS** y almacenamiento **local**.
...
> Idea clave: En dinámico, “pedimos y se crea”. En estático, “usamos lo que ya existe”.
...
- Consideraciones: Definimos una SC por perfil (rendimiento, costes, HA); usamos `allowVolumeExpansion: true`; marcamos una SC por defecto si procede.
...
- ¿Cuándo? 
  ...
  - Restauraciones/migraciones donde reaprovechamos un disco concreto (attach por `volumeName`).
...
## Seguridad (DevSecOps)
- Cifrado en reposo: lo habilitamos en el backend (EBS/PD/Disks, Ceph, Longhorn).
...
- Pruebas: `fio` con perfiles representativos; revisamos IOPS/latencia/throughput.
...
## Ejercicios
1) Creamos un PV/PVC estático en NFS usando selector de labels y montamos un Pod de prueba.
2) Definimos una StorageClass con `WaitForFirstConsumer`; creamos un PVC y verificamos la zona donde se crea el volumen.
3) Implementamos Longhorn en 3 nodos, configuramos `numberOfReplicas: 2` y simulamos la caída de un nodo.
4) Desplegamos Rook‑Ceph y creamos SC de RBD y CephFS; probamos un Deployment (RWO) y otro (RWX).
5) Creamos un VolumeSnapshot y restauramos un PVC; medimos diferencias de rendimiento con `fio`.

## Conclusiones
- Usamos aprovisionamiento **dinámico** con StorageClass como opción por defecto; aporta consistencia y automatización.
- Recurrimos a **estático** cuando necesitemos reutilizar capacidad existente, local PV o integrarnos con backends legacy.
- Elegimos el backend según tamaño y requisitos: nube nativa (simplicidad), Longhorn (pequeños), Rook‑Ceph (medianos), NFS (RWX simple), local (latencia mínima).
- Diseñamos desde el inicio con topología, seguridad, snapshots y backups en mente.

## Recursos Adicionales
- [Documentación de Kubernetes: Volumes, PV/PVC, StorageClass y Snapshots](https://kubernetes.io/docs/concepts/storage/)
- [Documentación de Longhorn](https://longhorn.io/docs/)
- [Documentación de Rook-Ceph](https://rook.io/docs/rook/latest/)
- [Velero (backup y DR)](https://velero.io/docs/)
