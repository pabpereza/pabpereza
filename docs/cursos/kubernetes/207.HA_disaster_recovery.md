...
- **Contras**: si cae un nodo, perdemos a la vez un API server y un miembro de etcd (los fallos van emparejados).
...
1. **Necesitamos un balanceador** delante de los API servers (HAProxy, keepalived, o el balanceador de nuestra nube) escuchando en el puerto 6443. El `--control-plane-endpoint` que ya usamos apunta a él, no a un nodo concreto. Esta es la razón por la que siempre lo configuramos como un nombre DNS: nos permite pasar de 1 a N masters sin regenerar el cluster.
...
- **Réplicas repartidas**: varios replicas con [anti-affinity o topology spread constraints](./121.Scheduling_labels.md) para que no compartan nodo o zona.
- **Probes**: [liveness y readiness](./113.Probes_live_readiness.md) para que el tráfico solo llegue a pods sanos.
...
Y tan importante como hacer backups: **probar la restauración**. Un plan de DR que nunca se ha ensayado es una hipótesis, no un plan. Practiquemos el ciclo completo (snapshot → cluster nuevo → restore → aplicaciones arriba) en un entorno de laboratorio.

### Métricas de un plan de DR
Dos siglas que conviene conocer (y que aparecen en cualquier conversación seria de DR):
- **RPO** (Recovery Point Objective): cuántos datos podemos permitirnos perder. Lo define la frecuencia de nuestros backups.
- **RTO** (Recovery Time Objective): cuánto tiempo podemos estar caídos. Lo define lo automatizado que esté nuestro procedimiento de restauración.
