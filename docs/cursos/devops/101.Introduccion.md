...
La definición corta que más me gusta: **DevOps es lo que hacemos para que desplegar deje de dar miedo.**
...
Con síntomas que reconoceremos si hemos trabajado así: releases nocturnas de fin de semana, documentos de despliegue de cuarenta páginas, y una reunión post-incidente dedicada a decidir de quién había sido la culpa.
...
**Medición.** No podemos mejorar lo que no medimos, y no podemos discutir con datos si no los tenemos.

**Compartir.** Que el conocimiento circule en lugar de acumularse en las personas.

Fijémonos en que solo uno de los cuatro es técnico. Es la razón por la que tantas transformaciones DevOps se quedan a medias: se compran herramientas y se ignoran los otros tres.
...
|---|---|---|
| **Frecuencia de despliegue** | Cada cuánto llegamos a producción | Varias veces al día |
| **Lead time for changes** | De commit a producción | Menos de un día |
| **Change failure rate** | % de despliegues que causan un fallo | 0-15% |
| **Time to restore** | Cuánto tardamos en recuperarnos | Menos de una hora |
...
No es magia. Si desplegamos una vez al trimestre, cada release acumula cientos de cambios: cuando algo falla, buscar la causa entre todos ellos es una investigación. Si desplegamos veinte veces al día, cada release lleva un puñado de cambios y el culpable es evidente.
...
Lo importante del diagrama es que **se cierra**. Lo que aprendemos monitorizando en producción alimenta lo que planificamos después. Un proceso que termina en "desplegado" y no vuelve al principio no es DevOps, es una cadena de montaje.
...
Lo vemos resumido en el [capítulo de seguridad](204.Seguridad.md) de este curso, y desarrollado con laboratorios en el [curso de DevSecOps](../devsecops/README.md).
...
Un consejo antes de seguir: **no intentemos implantarlo todo a la vez**. Elijamos el punto que más duele hoy —normalmente el despliegue manual—, arreglémoslo y midamos la diferencia. Es lo que convence a los escépticos.
...
- El ciclo se cierra: lo que aprendemos en producción alimenta lo que planificamos
...
> 💡 **Recuerda**: si nuestro último despliegue requirió una reunión previa y un fin de semana, ya sabemos por dónde empezar.
