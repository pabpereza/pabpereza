...
En el [capítulo anterior](./119.Usuarios.md) creamos identidades (usuarios y service accounts), pero por sí solas no pueden hacer nada en el cluster. RBAC (Role Based Access Control) es el mecanismo de autorización con el que les damos permisos.
...
Los **verbos** son las operaciones permitidas: `get`, `list` y `watch` para lectura; `create`, `update`, `patch` y `delete` para escritura. Los **apiGroups** indican el grupo del recurso: `""` para el core (pods, services...), `apps` para deployments y statefulsets, `batch` para jobs y cronjobs, etc. Si dudamos del grupo de un recurso, `kubectl api-resources` nos lo dice.
...
- Apliquemos el **principio de mínimo privilegio**: empecemos sin permisos y añadamos solo lo necesario.
- Evitemos los wildcards (`*`) en verbos, recursos o apiGroups, salvo en roles de administración deliberados.
- Vinculemos roles a **grupos** en lugar de a usuarios individuales cuando sea posible.
- Nunca demos `cluster-admin` a una aplicación ni a la service account `default`.
- Auditemos los permisos periódicamente con `kubectl auth can-i --list`.
