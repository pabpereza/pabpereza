...
keywords:
  ...
  - orquestación de contenedores local
  - variables entorno docker compose
  - docker compose tutorial paso a paso
description: >-
...
title: >-
  ...
  contenedores
sidebar_label: 12. Docker Compose
...
## Instalación
En Docker Desktop ya viene instalado, si utilizamos docker engine en nuestro ordenador con linux o servidores, podemos instalarlo siguiendo la documentación oficial:
...
(might require making the downloaded file executable with chmod u+x)
```


En el vídeo de la lista de reproducción de Docker, se muestra el proceso por si tenemos dudas.
...
Aun así, docker compose tiene bastantes más comandos pero vamos a empezar por los más básicos y esenciales. Dejamos la documentación oficial para poder consultarla si tenemos dudas o curiosidad: [Documentación CLI compose](https://docs.docker.com/reference/cli/docker/compose/) y [Documentación Docker Compose](https://docs.docker.com/compose/)
...
Como ejemplo más completo, vamos a ver cómo desplegar un wordpress con una base de datos mariadb. Este, añade también la exposición de varios puertos para la base de datos que solo serán accesibles desde la red interna de docker, gracias al comando `expose`.
...
También vemos la carga de variables de entorno y la política de `restart` que se aplica a los servicios. En este caso, se reiniciarán siempre que se detengan. ¿Nos suena de algo? Sí, es el flag `--restart` que usábamos en `docker run`. Podemos repasarlo todo en la sección [Ejecutar contenedores](104.Ejecutar_un_contenedor.md)
...
Si nos hemos quedado con ganas de más, tenemos más ejemplos en el repositorio de [Awesome Compose](https://github.com/docker/awesome-compose)
