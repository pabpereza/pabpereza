...
Git no es solo donde guardamos el código. Es el **disparador de todo el pipeline**: un push lanza los tests, un merge despliega a staging, un tag publica una release.

Por eso la estrategia de ramas que elijamos no es una preferencia estética. Determina con qué frecuencia podemos desplegar.
...
Su problema para DevOps es estructural: **está pensado para releases planificadas**, con versiones que se preparan y se publican cada cierto tiempo. Si queremos desplegar varias veces al día, la ceremonia de ramas sobra.

Sigue teniendo sentido en un caso concreto: software que se distribuye y del que mantenemos varias versiones a la vez, como una aplicación de escritorio o una librería con soporte a versiones antiguas.
...
Es lo que queremos para entrega continua, y la opción por defecto para la mayoría de los equipos que despliegan servicios web.
...
Elijamos por la frecuencia de despliegue que necesitamos, no por lo que hace la empresa de al lado.
...
Un buen candidato para un hook de pre-commit es la detección de secretos, que tenemos con laboratorio en el [capítulo 2 del curso de DevSecOps](../devsecops/102.Secretos_en_codigo.md).

## Rebase o merge

Una discusión eterna que tiene una respuesta razonable:

- **Rebase** en nuestra rama, antes de abrir el pull request, para tener un historial limpio y lineal
- **Merge** para integrar en `main`, para conservar el contexto de que esos commits eran una unidad

Y la regla que no se negocia: **nunca hagamos rebase de algo que ya hemos compartido**. Reescribir commits que otros tienen en su copia local es la forma más rápida de arruinarle la mañana a alguien.
...
Sin firma, cualquiera puede hacer un commit con nuestro nombre y nuestro correo: Git no lo comprueba. En proyectos abiertos o con requisitos de trazabilidad, la firma es lo que convierte la autoría en algo verificable.

## Resumen

- La estrategia de ramas determina con qué frecuencia podemos desplegar
...
- Rebase en nuestra rama, merge para integrar, y nunca rebase de lo compartido
- Protejamos `main`, incluida la exigencia de estar actualizada antes de fusionar
...
> 💡 **Recuerda**: si nuestras ramas viven semanas, no estamos integrando de forma continua por muchos jobs en verde que tengamos.
