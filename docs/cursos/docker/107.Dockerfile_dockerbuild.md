...
keywords:
  ...
  - crear imágenes docker desde cero
  - tutorial dockerfile paso a paso
  - docker build cache
sidebar_label: 7. Dockerfiles y Build
slug: dockerfiles_y_docker_build_construccion_de_imagenes
---

# Dockerfiles y Docker Build
En este apartado vamos a ver cómo se construyen las imágenes utilizando docker build y las instrucciones esenciales de un Dockerfile para empaquetar nuestra aplicación o servicio a un contenedor.
...
## Dockerfile
Si queremos refrescar que es un dockerfile, podemos ver la sección de [Conceptos básicos de Docker](103.Conceptos_basicos.md)
...
Ya vemos lo simple que es consumir imágenes oficiales de productos finales como Nginx. Solo necesitamos copiar nuestros archivos al directorio adecuado y listo.
...
Como vemos, hemos añadido una instrucción `RUN` que ejecuta el comando `pip install -r requirements.txt` para instalar las dependencias de nuestro script.
...
Si nos interesa profundizar en el tema de optimización de Dockerfiles, recomiendo que investiguemos sobre las las multi-stage builds, habrá un capítulo dedicado a ello más adelante y [tengo una lista de reproducción en Youtube sobre el tema:](https://www.youtube.com/playlist?list=PLQhxXeq1oc2mB6_KY-l_zgWJWZo_ne9MZ)
...
Vamos a probar a construir las imágenes de los Dockerfiles anteriores. Para ello, creamos un directorio y copiamos los ficheros `index.html` y `Dockerfile` del primer ejemplo y otro directorio para los ficheros `script.py`, `requirements.txt` y `Dockerfile` del segundo ejemplo.
...
Si todo ha ido bien, ya deberíamos tener nuestras imágenes construidas. Podemos comprobarlo con el comando `docker images`.
...
Como podemos ver, el nombre de la imagen sirve para referenciarla a la hora de ejecutar el contenedor, descargarla o subirla a un registro de imágenes.
