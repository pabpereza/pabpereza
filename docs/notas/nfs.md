...
Veamos las diferentes opciones que podemos configurar por cada origen o grupo de orígenes: * Permisos de lectura: `rw` para lectura y escritura y `ro` para solo lectura. * Opciones de sincronización: `sync` para sincronizar los archivos y `async` para no sincronizar los archivos. * Opciones de compartición: `no_subtree_check` para que no se compruebe el subdirectorio y `no_root_squash` para que no se comparta el root.
