...
│   ├── webservers.yml   # Solo para el grupo webservers
│   └── databases.yml    # Solo para el grupo databases
└── host_vars/
...
**`group_vars/webservers.yml`** — parámetros comunes del grupo:
```yaml
http_port: 80
...
```yaml
http_port: 8080      # target1 corre en un puerto no estándar
max_workers: 8       # Tiene más CPUs
```
...
- hosts: webservers
  tasks:
    - name: Configurar Nginx
      ...
      # {{ http_port }} y {{ max_workers }} vienen de group_vars/host_vars
```

Ansible fusiona automáticamente las variables. `target1` usará `http_port: 8080` porque `host_vars` es más específico que `group_vars`.

### Separar secrets con `vars_files`

Para no mezclar contraseñas con configuración:

```yaml
- hosts: databases
  vars_files:
    - vars/common.yml
    - vars/secrets.yml    # Este va en .gitignore o se cifra con Vault
  tasks:
...
| `ansible_fqdn` | `target1.ejemplo.com` |
| `ansible_date_time.date` | `2025-09-01` |
...
- name: Mostrar metadatos del host actual
  debug:
    msg: |
      ...
      Todos los hosts del grupo webservers: {{ groups['webservers'] }}
...
Aplicamos todo lo visto en un playbook que se comporta diferente según el entorno (dev, staging, prod) sin duplicar código.
...
```jinja2
# Generado por Ansible para {{ inventory_hostname }} ({{ group_names | join(', ') }})
app_name = {{ app_name }}
port = {{ app_port }}
...
- name: Desplegar aplicación
  ...
  tasks:
    - name: Mostrar entorno detectado
      debug:
        msg: "Desplegando en {{ group_names }} con {{ ansible_processor_vcpus }} vCPUs"

    - name: Calcular workers óptimos según CPUs
      set_fact:
        workers: "{{ [workers, ansible_processor_vcpus * 2] | min }}"
      when: workers | int > ansible_processor_vcpus * 2

    - name: Generar configuración
      template:
        src: templates/app.conf.j2
        dest: /etc/{{ app_name }}/app.conf
        validate: "/opt/{{ app_name }}/bin/validate-config %s"
      notify: Restart app
...
# Producción con override manual
ansible-playbook -i inventory/prod.ini site.yml -e "workers=16"
