...
En el [capítulo de deployments](./107.Deployments.md) vimos las estrategias nativas: `RollingUpdate` y `Recreate`. Funcionan, pero comparten una limitación: la nueva versión se va liberando a **todos** los usuarios a la vez, sin fase de prueba con tráfico real. En este capítulo veremos las dos estrategias que resuelven eso, y cómo implementarlas con los recursos que ya conocemos.

## Recapitulando: el problema
Un rolling update sustituye pods viejos por nuevos progresivamente. Si la versión nueva tiene un bug que solo aparece con tráfico real, lo descubriremos cuando ya esté sirviendo a buena parte de nuestros usuarios. Las estrategias avanzadas atacan ese hueco:
...
**Inconvenientes**: duplicamos recursos durante la transición, y cuidado con las migraciones de base de datos (ambas versiones conviven apuntando al mismo dato).
...
Con 9 réplicas de `app-stable` (v1) y 1 de `app-canary` (v2), aproximadamente el 10% del tráfico irá a la versión nueva. Para promover, escalamos:
```bash
kubectl scale deployment app-canary --replicas=5   # ~36%
kubectl scale deployment app-stable --replicas=0   # 100% canary
```

Es la implementación que pide el examen CKAD: dos deployments + un service, controlando el reparto con las réplicas. Su limitación obvia: el porcentaje va atado al número de pods (para un 1% necesitamos 99+1 réplicas).

### Canary con pesos: Gateway API
Como vimos en el [capítulo de Gateway API](./112.Gateway.md), el reparto por pesos es parte del estándar y desacopla el tráfico del número de réplicas:
...
Y combinándolo con matching por headers, podemos mandar a la versión nueva solo a nuestro equipo de QA antes de abrir el grifo. Si usamos ingress-nginx, existe el equivalente con anotaciones (`nginx.ingress.kubernetes.io/canary: "true"` y `canary-weight`), aunque ya sabemos lo que opinamos de las anotaciones propietarias.

### Canary automatizado: progressive delivery
En producción seria, el ciclo "subir peso → mirar métricas → subir más o abortar" no se hace a mano. Herramientas como **Argo Rollouts** o **Flagger** lo automatizan: definimos los pasos (5% → 20% → 50% → 100%), las métricas de éxito (tasa de errores, latencia desde Prometheus) y el sistema promueve o revierte solo. Es lo que se conoce como *progressive delivery*, y aunque queda fuera del examen, es el estándar de facto en la industria.

## ¿Qué estrategia elegir?
- **Rolling update**: nuestra opción por defecto. Simple, nativa, suficiente para la mayoría de servicios.
- **Blue-green**: cuando necesitamos validación completa antes del cambio y rollback instantáneo (releases de riesgo, ventanas de mantenimiento cortas).
- **Canary**: cuando queremos validar con tráfico real limitando el radio de impacto. Imprescindible en servicios de alto tráfico.
