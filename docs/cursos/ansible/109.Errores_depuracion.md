---
title: Errores y depuración en Ansible - block, rescue y debug
description: >-
  Depura playbooks de Ansible con block, rescue, always, failed_when, el
  módulo debug y los modos --check y -vvv. Errores comunes y cómo
  diagnosticarlos.
keywords:
...
Cómo hacer que nuestros playbooks sean resistentes a fallos y reaccionen de forma inteligente.
...
Cuando un playbook falla, Ansible se detiene inmediatamente. Eso puede ser catastrófico si estamos a mitad de un despliegue en producción. Necesitamos mecanismos para:
...
A veces Ansible piensa que algo falló cuando en realidad está todo bien, o viceversa. Con `failed_when` decidimos qué es un fallo.
...
Cuando desplegamos en múltiples servidores, a veces necesitamos que si **uno** falla, se detengan **todos**. Es la diferencia entre un fallo parcial controlado y un desastre.
...
- **Bases de datos** en modo réplica (si el primario falla, no toquemos los secundarios)
- **Balanceadores de carga** donde necesitamos al menos N servidores sanos


### `retries` y `delay`: reintentos inteligentes

A veces un servicio necesita unos segundos para arrancar. En lugar de fallar, reintentamos.
...
Cómo encontrar y solucionar problemas cuando nuestros playbooks no funcionan como esperábamos.

### La realidad: las cosas fallan

No importa lo buenos que seamos, nuestros playbooks van a fallar. La diferencia entre un principiante y un profesional es la **velocidad a la que diagnostican y resuelven** el problema.

#### 🕵️ La analogía: el detective

Depurar es como resolver un caso policial:
1. **Examinamos la escena del crimen** (los logs de error)
2. **Buscamos pistas** (verbose mode, debug)
3. **Interrogamos a los sospechosos** (variables, facts, conexión SSH)
4. **Reconstruimos los hechos** (paso a paso)
...
Con `-v` pudimos ver el mensaje de error completo que nos reveló el problema.


### El módulo `debug`: nuestro mejor amigo

El módulo `debug` es el equivalente a `console.log()` o `print()`. Nos permite inspeccionar variables en cualquier punto del playbook.
...
Ansible nos preguntará antes de cada tarea:
...
Es muy útil cuando el playbook falla a mitad de camino y queremos reiniciar desde donde falló sin repetir todo.
...
Cuando algo falla, seguimos este proceso ordenado:
...
A continuación tenemos un playbook con **varios errores intencionados**. Nuestra misión es encontrarlos y arreglarlos usando las técnicas de este capítulo.
