---
title: Ansible en CI/CD - Jenkins, GitHub Actions y despliegues
description: >-
  Integra Ansible en tus pipelines. Ejecuta playbooks desde Jenkins y GitHub
  Actions, gestiona secretos y aplica despliegues rolling, blue-green y
  canary.
keywords:
  - ansible cicd
  - ansible jenkins
  ...
  - ansible workflow
sidebar_label: 21. Ansible en DevOps y CI/CD
sidebar_position: 21
tags:
  - ansible
  - cicd
...
Este capítulo forma parte del bloque **Ansible Intermedio**, fuera del núcleo introductorio de 10 vídeos (capítulos 1-10). A partir de aquí el curso continúa con contenido de nivel avanzado.
:::
...
1. **Nunca** comiteamos `vault_password.txt` ni `.env` en el repo.
2. Usamos el **almacén de secretos del CI** (Jenkins Credentials, GitHub Secrets, HashiCorp Vault).
3. Pasamos la contraseña de Ansible Vault como variable de entorno: `ANSIBLE_VAULT_PASSWORD_FILE=...`.
4. Usamos claves SSH **dedicadas** al CI, con permisos mínimos (idealmente vía bastion + cert SSH).
5. Aplicamos `no_log: true` en tareas que manejen credenciales.
...
GitHub Actions es la opción más sencilla si nuestro código ya vive en GitHub. No necesitamos infraestructura propia: los runners hosted ya traen Python.
...
Ansible nos da control fino sobre **cómo** se aplican los cambios. Las cuatro estrategias clásicas:

### Rolling update

Despliega de N en N para no tirar el servicio. Es el comportamiento por defecto si usamos `serial`:
...
Mantenemos dos entornos idénticos (`blue` y `green`) y movemos el tráfico de uno a otro de golpe.
...
Para el resto, eliminamos el contenedor y volvemos a crearlo desde cero. **Nunca en prod** (causa downtime completo).
...
| Mantenimiento | Nosotros gestionamos el master + agentes | Cero |
...
- **Lint y test** de nuestros playbooks antes de desplegar (`ansible-lint`, `molecule`).
...
- **Logs estructurados**: usamos `ANSIBLE_LOG_PATH` y los archivamos como artefacto del pipeline.
- **Notificaciones**: integramos Slack, Teams o Telegram en el `post` del pipeline.
...
Construimos una pipeline completa que:
...
Si llegamos hasta aquí, ya tenemos un setup de DevOps real basado en Ansible. 🎉
...
**Hemos terminado el curso de Ansible.** 🎓 Volvemos al [README](./README.md) o nos pasamos por el [canal de YouTube](https://www.youtube.com/@Pabpereza) para seguir aprendiendo.
