...
Un bucket mal configurado en la consola de AWS lo descubres cuando alguien lo encuentra. El mismo bucket en un `main.tf` lo descubres en el pull request, treinta segundos después de escribirlo y antes de que exista.
...
Fíjate en el último: **Checkov también busca secretos**. Ha encontrado la contraseña de la base de datos que ya nos marcó gitleaks en el capítulo 2. Tres herramientas distintas, el mismo hallazgo — y eso está bien, porque no todo el mundo ejecuta las tres.

## Lo que Checkov no puede saber

Checkov comprueba buenas prácticas universales: cifra los buckets, no abras el SSH al mundo, activa los logs. Todo eso vale para cualquier empresa.

Lo que no sabe son **tus** reglas:
...
## Laboratorio: tu propia política

Usamos [Conftest](https://www.conftest.dev/), que aplica políticas de OPA a ficheros de configuración.

Antes de escribir nada, conviene mirar cómo ve Conftest tu Terraform:
...
El detalle que hace perder media hora a todo el mundo: **cada recurso es una lista**, no un objeto. Por eso las reglas llevan un `[_]` al final. Si tu política pasa cuando debería fallar, es casi siempre esto.
...
tfsec se fusionó con Trivy, así que si ya usas Trivy para imágenes y dependencias, `trivy config` te cubre la IaC sin añadir herramientas:
...
Ese `soft_fail: true` es deliberado. Un Terraform con años encima puede dar cientos de hallazgos el primer día, y bloquear de golpe solo consigue que te pidan quitar el control.
...
- Tus reglas de negocio no las conoce ninguna herramienta: eso es Policy as Code con Rego
- En Conftest, **cada recurso es una lista**: sin el `[_]` la política pasa en silencio
- Si ya usas Trivy, `trivy config` te ahorra una herramienta
- Empieza informando, congela la deuda y bloquea cuando sea manejable
