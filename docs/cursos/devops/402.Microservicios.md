...
Es una decisión con consecuencias enormes y difíciles de revertir, así que este capítulo empieza por la parte que casi nunca se cuenta: **por qué probablemente no los necesitamos**.
...
El problema que resuelven de verdad es **organizativo**. Cuando tenemos ochenta desarrolladores tocando el mismo repositorio, coordinar releases se convierte en el cuello de botella. Los microservicios permiten que quince equipos desplieguen sin pedirse permiso.

Es la ley de Conway aplicada a propósito: la arquitectura acaba pareciéndose a la estructura de nuestra organización, así que más vale elegir la estructura antes.

**Si tenemos menos de veinte desarrolladores, es muy probable que no tengamos ese problema.**
...
Fijémonos en la segunda fila, que es la que más duele. En un monolito, "cobrar y reservar stock" es una transacción: o pasan las dos cosas o ninguna. Repartido en dos servicios, eso deja de existir y tenemos que construirlo a mano.
...
Señales bastante fiables de que nos vamos a arrepentir:

- **El equipo es pequeño.** Menos de quince o veinte personas: el coste de coordinación que resuelven no lo tenemos.
- **No tenemos observabilidad.** Sin trazas y logs centralizados, cada incidente será una investigación a ciegas.
- **No tenemos CI/CD sólido.** Si desplegar un servicio es manual, desplegar veinte es inviable.
- **El dominio no está claro.** Si aún no sabemos dónde están las fronteras del negocio, las vamos a poner mal — y mover una frontera entre servicios cuesta muchísimo más que mover un módulo.
- **Lo hacemos porque lo hace Netflix.** Netflix tiene problemas que nosotros no tenemos, y equipos de plataforma que tampoco.

### La alternativa razonable: el monolito modular

Un monolito con fronteras internas bien definidas nos da casi todas las ventajas organizativas sin la complejidad operativa:
...
Y tiene una propiedad valiosa: si un día de verdad necesitamos separar, **ya sabemos por dónde cortar**. Es mucho más fácil extraer un servicio de un monolito modular que arreglar unos microservicios mal divididos.

Empecemos aquí. Separemos cuando el dolor sea real y podamos señalarlo con el dedo.

## Cómo dividir

Si decidimos seguir adelante, la división correcta es **por dominio de negocio**, no por capa técnica:
...
## Los patrones que vamos a necesitar
...
La regla práctica: **síncrono solo cuando de verdad necesitamos la respuesta ahora**. Un pedido no necesita esperar a que se envíe el email de confirmación.
...
Si nos faltan tres o más, resolvámoslos antes. Son más baratos de montar con un monolito que en medio de una migración.

## Resumen

- Los microservicios resuelven un problema **organizativo**, no técnico
- Con menos de veinte desarrolladores, probablemente no tenemos ese problema
...
- Asíncrono por defecto; síncrono solo si necesitamos la respuesta ya
...
> 💡 **Recuerda**: los microservicios cambian complejidad de código por complejidad de operación. Asegurémonos de que ese cambio nos compensa.
