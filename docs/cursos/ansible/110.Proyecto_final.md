---
title: Proyecto Ansible - despliega una app real en 3 servidores
description: >-
  Proyecto final del curso de Ansible. Despliega una app en tres servidores
  con Nginx como balanceador, Docker Compose, secretos en Vault y firewall
  UFW.
keywords:
  - proyecto ansible
...
sidebar_label: 10. Proyecto final
sidebar_position: 10
tags:
  - ansible
  - proyecto
...
Hemos llegado al final del curso. Es hora de juntar todo lo aprendido en un proyecto que simula una infraestructura real: tres servidores, balanceo de carga, secretos cifrados y firewall bastionado, todo automatizado con Ansible.
...
Tenemos tres servidores (`target1`, `target2`, `target3`) y necesitamos desplegar la aplicación [pabpereza/quotes](https://github.com/pabpereza/quotes) con alta disponibilidad básica:
...
El template itera sobre `groups['madrid']` para construir el upstream automáticamente. Si mañana añadimos `target4` al grupo `madrid`, nginx lo incluye sin tocar ningún fichero.
...
- **madrid**: puerto 8000 accesible (en producción restringiríamos el origen a la IP de target1)
...
| Tags, `import_playbook` | [21](201.CICD.md) | Tags `plataformado`, `despliegue`, `bastionado`; `site.yml` como orquestador |
