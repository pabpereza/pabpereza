---
title: Módulos avanzados de Ansible - template Jinja2, lineinfile y check mode
description: >-
  Aprende los módulos de Ansible para producción: template con Jinja2,
  lineinfile y blockinfile, command/shell idempotentes con changed_when y
  validación con --check y --diff. Curso gratuito en español.
keywords:
...
sidebar_label: 4. Módulos avanzados e idempotencia
sidebar_position: 4
tags:
  - ansible
  - modulos
...
# Módulos avanzados de Ansible: template, lineinfile y check mode

En el módulo anterior aprendimos los módulos esenciales y el concepto de idempotencia. Ahora toca profundizar: ¿qué pasa cuando los módulos *no* son idempotentes? ¿Cómo generamos configuraciones dinámicas? ¿Cómo cirugía un archivo de config sin reemplazarlo entero?


[![Idempotencia y módulos avanzados para producción - Curso Ansible #4](https://img.youtube.com/vi/U7a61YZoqB0/maxresdefault.jpg)](https://youtu.be/U7a61YZoqB0)

## Idempotencia en la práctica: `--check` y `--diff`

Antes de tocar nada en producción, necesitamos responder a la pregunta: **¿qué va a cambiar exactamente?**
...
Cuando combinamos ambos, Ansible nos dice exactamente qué modificaría sin tocar nada. Esto es nuestra red de seguridad antes de cada deploy.

### La regla de oro de la idempotencia

> Un playbook bien escrito debe terminar con `changed=0` cuando lo ejecutamos **dos veces seguidas**. Si el segundo run reporta cambios, tenemos un problema de idempotencia.
...
## `ansible-doc`: nuestra documentación offline

No necesitamos internet para consultar qué hace cada módulo. Ansible incluye documentación completa en local:
...
Lo usamos antes de buscar en Google. Suele ser más preciso y funciona sin conexión.


## `template`: de archivos estáticos a configuraciones dinámicas

En el módulo anterior usamos `copy` para subir archivos. `copy` es estático: lo que sube es exactamente lo que hay en nuestro PC. `template` es su hermano mayor: procesa el archivo con **Jinja2** antes de subirlo, permitiendo usar variables, condicionales y loops dentro del propio archivo de configuración.

### La analogía: la plantilla de Word

Imaginemos una plantilla de carta formal donde cambiamos el nombre del destinatario, la fecha y el motivo. El cuerpo es siempre el mismo, pero los datos son dinámicos. Eso es exactamente lo que hace `template` con nuestras configs.
...
Hay tres constructores que usaremos el 90% del tiempo:
...
También sabemos que dentro de un template Jinja2 podemos usar cualquier variable de Ansible, incluyendo facts del sistema (`ansible_facts['distribution']`) o variables definidas en el playbook (`http_port`).
...
A veces no queremos reemplazar un archivo entero, solo modificar una línea específica o añadir un bloque. Para eso están estos dos módulos.

### `lineinfile`: gestionar una línea

Asegura que una línea concreta existe (o no) en un archivo. Usamos `regexp` para encontrar la línea existente y `line` para reemplazarla.
...
Cuando necesitamos insertar o actualizar varias líneas a la vez. Ansible añade marcadores automáticos para saber dónde está el bloque y poder actualizarlo de forma idempotente.
...
Si volvemos a ejecutar el playbook, Ansible encuentra los marcadores, compara el contenido y solo actúa si el bloque es diferente. Idempotente.
...
Le decimos a Ansible: "ejecuta este comando solo si este archivo NO existe" (`creates`) o "solo si SÍ existe" (`removes`).
...
Capturamos la salida del comando y definimos nosotros mismos cuándo se considera un cambio.
...
Algunos comandos nos dicen si hicieron cambios en su output:
...
| Necesitamos resultado para decidir | `register:` + `when:` en task siguiente |
...
* **`--check --diff`:** Simula cambios y muestra diffs antes de aplicar. Nuestra red de seguridad.
...
**Próximo paso:** Variables, facts y plantillas más potentes para hacer nuestros playbooks verdaderamente dinámicos y reutilizables en cualquier entorno.
