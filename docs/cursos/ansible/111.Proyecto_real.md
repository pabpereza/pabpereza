...
description: >-
  ...
  con base de datos PostgreSQL, backend Flask, proxy inverso Nginx y monitorización,
  usando roles, Vault e inventarios separados por entorno.
keywords:
  - proyecto ansible
  - ansible proyecto final
  - despliegue aplicacion ansible
  - ansible flask postgresql
  - ansible roles proyecto
  - notastack ansible
  - ansible produccion ejemplo
...
Aquí aplicamos lo aprendido en la [sección de Inventarios](102.Inventarios.md). Fíjate en cómo agrupamos los hosts por función y cómo usamos variables de grupo.
...
- [ ] `ansible all -m ping` responde `pong` en los tres nodos (target1, target2, target3)
...
Aquí aplicamos lo aprendido en la [sección de Roles y Templates Jinja2](106.Roles.md):
...
# Acceso desde la red de la aplicación (grupo madrid: target1 y target2)
...
Fíjate cómo el template de `pg_hba.conf` itera sobre los hosts del grupo `madrid` para dar acceso automáticamente. Si mañana añades otro servidor de aplicación al inventario, se configurará solo.
...
- [ ] PostgreSQL arranca correctamente en `target3`
...
Necesitamos la colección `community.postgresql` que ya usamos antes. Vamos a gestionar dependencias con un fichero de requisitos, como aprendimos en la [sección de Roles y Ansible Galaxy](106.Roles.md):
...
- [ ] La aplicación Flask arranca en `target2`
...
Aquí aplicamos lo aprendido sobre playbooks y orquestación. Separamos cada capa en su propio playbook:
...
- [ ] Nginx sirve la aplicación en el puerto 80 de `target1`
...
Proteger las contraseñas y datos sensibles con Ansible Vault, como aprendimos en la [sección de Seguridad](107.Seguridad.md).
...
Hacer el despliegue resistente a fallos, aplicando técnicas de robustez del curso.
...
- name: Validaciones previas al despliegue
  ...
  tasks:
    ...
    - name: Verificar conectividad entre app y db
      command: "ping -c 1 -W 2 {{ hostvars[groups['barcelona'][0]]['ansible_host'] }}"
      changed_when: false
...
Crea un playbook que recoja información útil de toda la infraestructura para diagnóstico y depuración:
...
| [Playbooks](103.Playbooks.md) | Todos los ficheros en `playbooks/` |
| [Módulos](104.Modulos_idempotencia.md) | apt, systemd, template, uri, file, user, copy... |
| [Variables, condicionales y handlers](105.Variables_control_flujo.md) | `group_vars/`, defaults de roles, facts del sistema |
| [Roles y Templates Jinja2](106.Roles.md) | Cuatro roles independientes, configuraciones dinámicas y Galaxy |
| [Seguridad - Vault y credenciales](107.Seguridad.md) | Contraseñas cifradas en `vault.yml` y gestión de secretos |
| [Manejo de errores](108.Errores_depuracion.md) | Bloques block/rescue/always, assert, validaciones |
| [Include/Import y orquestación](108.Errores_depuracion.md) | `site.yml` como orquestador con import_playbook |
| [Depuración](108.Errores_depuracion.md) | Playbook de diagnóstico, debug, registros |
