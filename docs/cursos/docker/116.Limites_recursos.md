...
sidebar_label: 16. Límites y recursos
keywords:
  - docker
  - límites recursos docker
  - docker cpu limit
  - docker memory limit
  - cgroup docker
  - docker stats
  - controlar recursos contenedores
  - docker --memory
  - docker --cpus
  - rendimiento docker
  - optimizar recursos docker
  - docker compose limits
  - monitorizar recursos docker
  - docker run
...
Esto lo podemos hacer de varias formas, ya sea al crear el contenedor, con `docker run`, o en un fichero `compose.yml` utilizando `docker compose`. Aunque también podemos modificar estos límites en caliente con `docker update`.
...
Esto no significa que el contenedor solo consumirá 256MB de memoria, sino que garantiza que siempre tendrá al menos 256MB de memoria disponible.
