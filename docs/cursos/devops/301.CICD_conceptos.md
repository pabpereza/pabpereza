...
**La integración continua trata de integrar a menudo.** Si tenemos ramas que viven tres semanas antes de fusionarse, no estamos haciendo CI aunque tengamos cien jobs en verde. Estamos haciendo integración esporádica con validación automática, que está bien, pero no nos da lo que promete la CI.
...
**El artefacto se construye una sola vez.** Se construye en CI y ese mismo artefacto atraviesa staging y producción. Si reconstruimos para producción, lo que desplegamos no es lo que probamos.

**Fallar rápido.** Lo barato primero. Si el lint falla en veinte segundos, no gastemos cinco minutos de tests.

## Elegir plataforma

| | GitHub Actions | GitLab CI | Jenkins |
|---|---|---|---|
| **Instalación** | Nada, va integrado | Nada si usamos GitLab | Un servidor que mantener |
| **Configuración** | YAML | YAML | Groovy o interfaz web |
| **Ecosistema** | Enorme (marketplace) | Bueno | Enorme, pero desigual |
| **Coste** | Gratis en repos públicos | Minutos incluidos | Solo nuestra infraestructura |
| **Fuerte en** | Integración con GitHub | Todo en una herramienta | Flexibilidad extrema |

La recomendación honesta: **usemos el CI de donde ya está nuestro código**. La integración lo vale, y el tiempo que ahorramos en mantenimiento es real.

Jenkins sigue teniendo sentido en dos casos: necesitamos algo muy específico que las plataformas gestionadas no permiten, o tenemos requisitos de aislamiento que obligan a alojarlo nosotros. Fuera de eso, mantener un Jenkins es un trabajo a tiempo parcial que nadie quiere.
...
El despliegue continuo no es una herramienta que enchufamos, es una consecuencia. Requiere:

- ✅ Tests en los que el equipo confíe de verdad
- ✅ Despliegue automatizado, sin pasos manuales escondidos
- ✅ Monitorización que nos avise antes que los usuarios
- ✅ Rollback rápido y probado
- ✅ Feature flags para desacoplar desplegar de activar

Si nos falta el rollback o la monitorización, el despliegue continuo no nos hace ir más rápido: nos hace romper cosas más rápido.
...
- **Reconstruir por entorno.** Rompe la garantía de que desplegamos lo que probamos.
...
- **Construyamos el artefacto una vez** y promocionémoslo entre entornos
- Usemos el CI de donde ya vive nuestro código
...
> 💡 **Recordemos**: el objetivo de CI/CD no es desplegar mucho, es que desplegar deje de dar miedo.
