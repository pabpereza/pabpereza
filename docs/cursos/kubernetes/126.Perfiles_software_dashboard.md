...
> **Aviso para certificaciones:** en los exámenes CKA, CKAD y CKS solo tendremos `kubectl` (con autocompletado y el alias `k`). Estas herramientas son para nuestro día a día profesional; no nos apoyemos en ellas para preparar el examen.

## Perfiles y gestión de múltiples clusters
En cuanto trabajemos en serio con Kubernetes, acabaremos teniendo varios clusters: el de desarrollo, el de producción, el minikube local... Como vimos en el [capítulo de manifiestos](./105.Conceptos.md), kubectl gestiona esto mediante **contextos** en el kubeconfig.
...
Una vez dentro, escribimos `:` seguido del recurso (`:deploy`, `:svc`, `:ns`...) para cambiar de vista. Sobre cualquier recurso podemos pulsar `l` para ver logs, `d` para describirlo, `e` para editarlo o `s` para abrir una shell en el contenedor. Todo lo que haríamos con varios comandos de kubectl, a golpe de tecla.

Si pasamos muchas horas administrando clusters, k9s amortiza su curva de aprendizaje el primer día.

## Lens y las GUIs de escritorio
Si preferimos una aplicación gráfica de escritorio, [Lens](https://k8slens.dev/) es la más conocida: muestra múltiples clusters, métricas, logs y permite editar recursos desde una interfaz muy pulida. Existe también [OpenLens](https://github.com/MuhammedKalkan/OpenLens), su variante totalmente open source, y alternativas como [Headlamp](https://headlamp.dev/) (proyecto sandbox de la CNCF) o [Aptakube](https://aptakube.com/).

Y recordemos que el [Dashboard oficial de Kubernetes](./124.Logging.md) que instalamos en el capítulo anterior cubre el caso de una interfaz web desplegada en el propio cluster.
...
Por último, no subestimemos lo lejos que se llega solo con la terminal bien configurada. Esto sí aplica a los exámenes:
...
Con esto cerramos la parte común del curso. A partir de aquí, elijamos nuestra especialización: [CKA (administración)](./201.Mantenimiento_backup.md), [CKAD (desarrollo)](./301.Pods_initcontainers.md) o [CKS (seguridad)](./401.CIS_benchmarks.md).
