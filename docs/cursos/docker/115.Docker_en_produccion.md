...
keywords:
  ...
  - seguridad docker produccion
  - resiliencia docker
  - health checks docker
  - restart policy docker
  - despliegue docker en servidor
  - optimización docker
  - docker logging producción
  - docker deployment
  - cómo usar docker en producción
description: >-
...
sidebar_label: 15. En producción
...
Nos gusta utilizar el instalador de Docker en un solo comando, recordemos que tenemos una sección dedicada a la instalación de Docker en el [capítulo 2](102.Instalacion.md).
...
Ojo, porque si usamos el ejemplo del episodio anterior, la imagen del API se construye desde el dockerfile por lo que este fichero deberá estar también en el servidor para que compose pueda construir la imagen.

Lo más sencillo creo que es clonar el repositorio entero en el servidor y lanzar el compose. Así, más adelante cuando queramos actualizar la aplicación, solo tenemos que hacer un git pull en el servidor y volver a lanzar el compose.
